#!/usr/bin/env python3
"""
Training script for drone tracking RL agent on real humans using YOLO.

Usage:
    python src/agents/train_yolo.py --steps 1000000 --save-dir ./models
"""
...
def main():
    parser = argparse.ArgumentParser(
        description="Train drone tracking agent on real humans"
    )
    parser.add_argument(
    ...
    print("=" * 60)
    print("Drone Tracking RL Training - YOLO Human Detection")
    print("=" * 60)
    ...
    try:
        ...
        env = Monitor(env, filename=str(monitor_dir / "training_yolo_monitor.csv"))
        ...
        if args.resume:
            ...
            model = RecurrentPPO(
                ...
                clip_range=0.2,
                ent_coef=0.01,
                tensorboard_log="./tensorboard_yolo/",
            )
        ...
        checkpoint_callback = CheckpointCallback(
            ...
            name_prefix="drone_tracking_yolo",
        )

        print(f"\n[*] Starting training for {args.steps} steps...")
        print(f"    Using YOLO human detection from camera feed")
        print(f"    Checkpoints saved to: {save_dir}")
        print(f"    TensorBoard logs: ./tensorboard_yolo/")
        ...
        # Save final model
        final_path = save_dir / "drone_tracking_yolo_final.zip"
